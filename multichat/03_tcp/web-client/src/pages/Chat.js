...
async function initializeChat(username) {
    ...
    initializeAudioService(username,
        ...
        (remoteUser) => {
            ...
            // Send fallback text message for signaling (in case WebSocket fails)
            if (remoteUser) {
                const payload = `INCOMING_CALL_ACCEPT|${Date.now()}`;
                setTimeout(async () => {
                    console.log('[UI] Sending fallback CALL_ACCEPT via REST to', remoteUser);
                    try {
                        const res = await sendMessageToUser(username, remoteUser, payload);
                        if (!res?.success) {
                            console.warn('[UI] Fallback CALL_ACCEPT REST send failed:', res?.message);
                        }
                    } catch (restErr) {
                        console.warn('[UI] Error sending fallback CALL_ACCEPT via REST:', restErr);
                    }
                }, 500);
            }
            ...
            if (currentCallInfo && currentCallInfo.state === 'connected') {
                ...
                // Force connected state for demo if using dummy mode
                updateCallUIStatus('In call');
                activeCallStartTime = Date.now();
            }
        },
        ...
        if (result.success && result.history) {
            ...
            result.history.forEach(entry => {
                try {
                    ...
                    // Skip control/call signaling messages from history
                    const control = classifyControlMessage(msg);
                    if (control.skip && !control.display) return;
                    const effectiveMsg = control.display || msg;
                    ...
                    const isDuplicate = messageCache[chatKey].some(m =>
                        m.from === messageFrom && m.content === effectiveMsg
                    );
...
function processIncomingMessage(msg) {
    ...
    if (parts[0] === 'MSG') {
        // Direct message
        const from = parts[1];
        const originalContent = parts.slice(2).join('|');
        const chatKey = `user_${from}`;
        const ts = new Date();

        console.log('Direct message from:', from, 'Current chat:', currentChat);

        // Normalize/control handling first
        const control = classifyControlMessage(originalContent);
        if (control.skip && !control.display) return;
        const content = control.display || originalContent;

        // Check if it's a special message type (file or audio)
        try {
            const parsedContent = JSON.parse(content);
            if (parsedContent.type === 'CALL_END') {
                console.log('[UI] Received CALL_END via REST/ICE from', from);
                try { endAudioCall(false); } catch (e) { /* ignore */ }
                activeCallPeer = null;
                activeCallStartTime = null;
                localHangupInitiated = false;
                localHangupPeer = null;
                updateCallUIStatus('Call ended');
                const statusText = '(Call ended)';
                const isDup = messageCache[chatKey]?.some(m => m.content === statusText);
                if (!isDup) {
                    if (!messageCache[chatKey]) messageCache[chatKey] = [];
                    messageCache[chatKey].push({ from, content: statusText, isSent: false, timestamp: ts });
                    if (currentChat && currentChat.type === 'user' && currentChat.name === from) {
                        addMessageToUI(from, statusText, false, ts);
                    }
                }
                return;
            }
            if (parsedContent.type === 'file') {
                ...
                return;
            } else if (parsedContent.type === 'CALL_REQUEST') {
                const caller = parsedContent.from || from;
                const offer = parsedContent.offer;
                const callId = parsedContent.callId || `${caller}_${sessionStorage.getItem('username')}_${Date.now()}`;
                if (offer) {
                    console.log('[UI] Received CALL_REQUEST via REST fallback from', caller);
                    handleCallStartedViaICE({
                        caller,
                        callId,
                        active: true,
                        offer
            ...
            } else if (parsedContent.type === 'CALL_ACCEPT' || parsedContent.type === 'call_log') {
                // Skip control/call log messages in chat view
                return;
            }
        } catch (e) {
        ...
        // Check if this is an INCOMING_CALL_ACCEPT message (Fallback)
        if (content.startsWith('INCOMING_CALL_ACCEPT|')) {
            console.log('[UI] Intercepted INCOMING_CALL_ACCEPT from', from);
            // Force UI to 'In call'
            updateCallUIStatus('In call');
            activeCallPeer = from;
            activeCallStartTime = Date.now();
            // Render a friendly status message instead of the raw payload
            const statusText = '(Call accepted)';
            const isDuplicateStatus = messageCache[chatKey]?.some(m =>
                m.from === from && m.content === statusText
            );
...
async function selectGroup(groupName) {
    ...
    if (messageCache[chatKey] && messageCache[chatKey].length > 0) {
        messageCache[chatKey].forEach(msg => {
            const control = classifyControlMessage(msg.content);
            if (control.skip && !control.display) return;
            const effectiveContent = control.display || msg.content;
            addMessageToUI(msg.from, effectiveContent, msg.isSent, msg.timestamp);
        });
    } else {
...
// Helper to classify control/call signaling messages so they don't pollute chat UI
function classifyControlMessage(rawContent) {
    const result = { skip: false, display: null };
    const text = typeof rawContent === 'string' ? rawContent : (rawContent != null ? String(rawContent) : '');

    if (text.startsWith('INCOMING_CALL|')) {
        result.skip = true;
        return result;
    }
    if (text.startsWith('INCOMING_CALL_ACCEPT|')) {
        result.display = '(Call accepted)';
        return result;
    }

    try {
        const parsed = JSON.parse(text);
        if (parsed && (parsed.type === 'CALL_REQUEST')) {
            result.skip = true;
            return result;
        }
        ...
        if (parsed && parsed.type === 'CALL_END') {
            result.display = '(Call ended)';
            return result;
        }
    } catch (e) {
        // not JSON, ignore
...
async function acceptCall(call) {
    ...
    try {
        ...
        // Fallback: notify caller via REST so their UI can flip to "In call" even if WS signaling fails
        try {
            const payload = `INCOMING_CALL_ACCEPT|${Date.now()}`;
            const res = await sendMessageToUser(username, call.caller, payload);
            if (!res?.success) {
                console.warn('[UI] Fallback CALL_ACCEPT REST send failed:', res?.message);
            }
        } catch (restErr) {
...
async function startCall() {
    ...
    try {
        ...
        const callData = await startAudioCall(callee);

        // Fallback: push CALL_REQUEST (with offer) via REST so callee gets the offer even if WS fails
        if (callData && callData.offer) {
            const callId = callData.callId || `${username}_${callee}_${Date.now()}`;
            const payload = {
                type: 'CALL_REQUEST',
                from: username,
                callId,
            ...
            try {
                const res = await sendMessageToUser(username, callee, JSON.stringify(payload));
                if (!res?.success) {
                    console.warn('[UI] Fallback CALL_REQUEST REST send failed:', res?.message);
                }
            } catch (restErr) {
                console.warn('[UI] Error sending fallback CALL_REQUEST via REST:', restErr);
            }
        }
        // Update UI status
...
async function hangupCall() {
    ...
    try {
        ...
        // REST fallback CALL_END so the other side cleans up even if WS/ICE fail
        if (username && activeCallPeer) {
            try {
                const payload = JSON.stringify({ type: 'CALL_END', from: username, to: activeCallPeer, durationMs });
                const res = await sendMessageToUser(username, activeCallPeer, payload);
                if (!res?.success) {
                    console.warn('[UI] Fallback CALL_END REST send failed:', res?.message);
                }
            } catch (restErr) {
                console.warn('[UI] Error sending fallback CALL_END via REST:', restErr);
            }
        }
        hideCallUI();
    } catch (error) {
